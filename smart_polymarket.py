...
st.info("**Production v3.1** — Clean Model with Sharp Comparison")
...
if ODDS_API_KEY:
    ...
    st.sidebar.info("Add THE_ODDS_API_KEY in secrets.toml for sharp edge")
...
def fetch_polymarket():
    ...
    for _ in range(50):
        try:
            resp = requests.get(f"{POLYMARKET_US_BASE_URL}/markets",
                              params={"active": "true", "limit": 100, "offset": offset}, timeout=25)
...
def fetch_sharp_odds():
    ...
    for sport in sports_list:
        try:
            r = requests.get(f"{THE_ODDS_API_BASE}/sports/{sport}/odds",
...
if df.empty:
    st.warning("No value bets found. The current sports markets on Polymarket are quite efficient.")
else:
